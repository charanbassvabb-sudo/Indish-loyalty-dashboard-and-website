...
import { PaymentVerificationResult } from "@/components/reservation/PaymentVerificationResult";

/**
 * Same shape as reservation.ts's PaymentUploadResult (see
 * ReservationPaymentStep.tsx) except `orderStatus` instead of
 * `reservationStatus` — mirrors uploadTakeawayPaymentScreenshot's response in
 * payment.controller.ts exactly.
 */
export interface TakeawayPaymentUploadResult {
  attemptId: number;
...
interface Props {
  ...
  reference: string;
  totalAmount: number;
  onVerified: (result: TakeawayPaymentUploadResult) => void;
}

/** "0979771033" -> "097 977 1033" */
...
/** Same UX as ReservationPaymentStep.tsx, targeting a takeaway order's payment-screenshot endpoint instead of a reservation's — same OCR/verification flow, same business numbers. */
export function TakeawayPaymentStep({ branchId, reference, totalAmount, onVerified }: Props) {
  ...
  const [result, setResult] = useState<TakeawayPaymentUploadResult | null>(null);
  const [previewUrl, setPreviewUrl] = useState<string | null>(null);
  const fileInputRef = useRef<HTMLInputElement>(null);
  const steps = instructionSteps(selected, totalAmount);
  ...
  async function handleFile(file: File) {
    ...
    try {
      const res = await api.upload<TakeawayPaymentUploadResult>(
        `/takeaway-orders/${reference}/payment-screenshot`,
        formData,
      );
      setResult(res);
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="card-warm flex items-center justify-between p-5">
        ...
        <span className="font-display text-2xl text-primary">ZMW {totalAmount}</span>
      </div>
      ...
      <div className="card-warm flex flex-col items-center gap-1 p-5 text-center">
        <span className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">
          {selected.isAgentNumber ? `Get Cash — pay ZMW ${totalAmount} to Agent` : `Send ZMW ${totalAmount} to`}
        </span>
